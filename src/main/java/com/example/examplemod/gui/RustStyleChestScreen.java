...
public class RustStyleChestScreen extends AbstractContainerScreen<ChestMenu> {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (titleStr.contains("도구함") || titleStr.toLowerCase().contains("cupboard")) {
            ...
            int centerX = this.leftPos + 430;
            int drawY = this.topPos + 148;
            String titleText = "§620분 유지보수 필요 자원";
            guiGraphics.drawString(this.font, titleText, centerX - (this.font.width(titleText) / 2), drawY, 0xFFFFFFFF, true);
            String costText = "§7" + maintenanceCost;
            guiGraphics.drawString(this.font, costText, centerX - (this.font.width(costText) / 2), drawY + 15, 0xFFFFFFFF, true);
            String statusText = isDecaying ? "§c건물 부식 중" : "§a건물 유지 중";
            int statusColor = isDecaying ? 0xFFFF0000 : 0xFF5DB31F;
            guiGraphics.drawString(this.font, statusText, centerX - (this.font.width(statusText) / 2), drawY + 30, statusColor, true);
