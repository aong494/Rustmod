    ...
    import com.example.examplemod.SyncExtraSlotPacket;
    ...
    public class RustStyleInventoryScreen extends AbstractContainerScreen<InventoryMenu> {
        ...
        private void handleCustomSlotClick(int index) {
            ...
            if (!carried.isEmpty() && !isItemValidForSlot(index, carried)) return;
            // 서버에 교체 요청
            ModMessages.sendToServer(new ArmorSwapPacket(index + 4));
            // [시뮬레이션] 서버 응답이 오기 전 마우스 아이템을 슬롯 아이템과 교체
            if (index == 0) {
        ...
        public void updateExtraSlots(ItemStack s0, ItemStack s1) {
            this.extraSlot0 = s0.copy();
            this.extraSlot1 = s1.copy();
        }
        private boolean isItemValidForSlot(int index, ItemStack stack) {
            return PlayerGearCapability.isItemValid(index, stack);
        }
        ...
        public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
            ...
            int barSpacing = 20; // 두 바 사이의 간격

            // 1. 보온(Insulation) 바 그리기
            drawStatBar(guiGraphics, statsX, statsYStart,
                    String.valueOf(com.example.examplemod.ClientStats.insulation), INSUL_ICON);

            // 2. 방호(Radiation) 바 그리기
            drawStatBar(guiGraphics, statsX, statsYStart + barSpacing,
                    String.valueOf(com.example.examplemod.ClientStats.radiation), RAD_ICON);
        ...
        private void drawStatBar(GuiGraphics guiGraphics, int x, int y, String value, ResourceLocation icon) {
            // [반투명 설정] RGBA (1.0f, 1.0f, 1.0f, 0.7f -> 70% 불투명도)
            RenderSystem.enableBlend();
            RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, 0.9f);

            // 1. 배경 PNG 그리기 (숫자와 아이콘을 감싸는 작은 바)
            guiGraphics.blit(STATS_BG, x, y, 0, 0, 80, 16, 80, 16);

            // 2. 숫자 그리기 (좌측 배치, 하얀색)
            // x + 5 위치에서 시작
            guiGraphics.drawString(this.font, value, x + 55, y + 4, 0xFFFFFFFF, true);

            // 3. 아이콘 그리기 (우측 배치)
            // 배경 너비(80)에서 아이콘 크기(12)와 여백을 뺀 위치
            RenderSystem.setShaderTexture(0, icon);
            guiGraphics.blit(icon, x + 65, y + 2, 0, 0, 12, 12, 12, 12);
